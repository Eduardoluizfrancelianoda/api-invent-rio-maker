...
class ComponenteSchema(BaseModel):
 ...
 quantidade: int = Field(..., ge=0, description="Quantidade em estoque (deve sermaior ou igual a zero)")
 categoria: str = Field(..., description="Categoria do item (ex: Atuadores,Microcontroladores)")
 estado_conservacao: str = Field(..., description="Estado de conservação do item (ex: Novo, Usado)")

# 3. Nosso "Banco de Dados" temporário em memória
estoque_laboratorio = [
 {"id": 1, "nome": "Arduino Sensor Shield", "quantidade": 15, "categoria": "Placasde Expansão", "estado_conservacao": "Novo"},
 {"id": 2, "nome": "Micro Servo Motor SG90", "quantidade": 42, "categoria":"Atuadores", "estado_conservacao": "Usado"},
 {"id": 3, "nome": "Esteira em Acrílico", "quantidade": 2, "categoria": "Mecânica", "estado_conservacao": "Novo"}
]

# Rota Raiz
@app.get("/")
def raiz():
 return {"mensagem": "API do Laboratório Maker operante. Acesse /docs para ver adocumentação."}
...
def remover_componente(componente_id: int):
 for index, item in enumerate(estoque_laboratorio):
    if item["id"] == componente_id: estoque_laboratorio.pop(index)
